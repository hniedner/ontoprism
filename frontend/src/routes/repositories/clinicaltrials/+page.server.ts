...
export const load: PageServerLoad = async ({ fetch, url }) => {
	...
	if (result.state === 'ready' && (result.data.page_size !== state.size || result.data.page_token !== cursor)) error(502, 'ClinicalTrials.gov page metadata did not match the request.');
