<script lang="ts" generics="Row">
	...
	let { rows, columns, getRowId, emptyMessage }: Props = $props();
...
<tbody>
	...
	{:else}
		<tr><td colspan={columns.length} class="border-b border-default/60 px-4 py-6 text-center text-sm italic text-muted">{emptyMessage}</td></tr>
	{/each}
</tbody>
