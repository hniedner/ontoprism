...
here makes `import fairlib` / `import backend` resolve to the real `*/src` packages —
ahead of the shadowing outer `fairlib/` & `backend/` directories — under prepend mode.
"""
...
for _src in ("fairlib/src", "backend/src"):
    _abs = str(_ROOT / _src)
