...
from test_support.publication_qlever import publication_marker, publication_qlever
...
async def _publication_size_contract(
    ...
    with publication_qlever(provision) as url:
...
async def test_small_publication_with_production_qlever_flags(
    ...
    assert await _publication_size_contract(
        tmp_path, qlever_resource_provisioner, 100, capsys
    ) == (105, False)
...
async def test_timed_out_promotion_reconciles_real_marker_and_counts(
    ...
    with publication_qlever(qlever_resource_provisioner) as url:
        async with SparqlHttpClient.for_qlever(url, query_timeout=60) as client:
            await client.load(
            ...
            for graph in (vocab.DECOMPOSED_GRAPH_IRI, staging):
                rows = await client.select_once(
                    f"SELECT (COUNT(*) AS ?n) WHERE {{ GRAPH <{graph}> "
                    "{ ?s ?p ?o } }",
                    required_variables={"n"},
                )
...
async def test_publication_loads_1_2gb_with_default_request_limit(
    ...
    assert await _publication_size_contract(
        tmp_path, qlever_resource_provisioner, 6_000_000, capsys
    ) == (6_000_005, False)
