...
@pytest.mark.unit
def test_expected_review_pairs_are_neither_credited_nor_penalized() -> None:
    # D-R1/D59 keeps engine flags in the strict denominator; only SME-deferred
    # expected pairs are excluded from scoring on both sides.
    golden = {("R88", "C27970"), ("R101", "C12400")}
    actual = {("R88", "C27970"), ("R101", "C12400"), ("R101", "C12418")}
    s = score(golden, actual, expected_needs_review={("R101", "C12418")})
    assert s.precision == 1.0
    assert s.recall == 1.0
    assert s.exact
...
@pytest.mark.unit
def test_sme_deferred_golden_pair_is_not_a_recall_miss() -> None:
    golden = {("R88", "C27970"), ("R101", "C12418")}
    s = score(
        golden,
        {("R88", "C27970")},
        expected_needs_review={("R101", "C12418")},
    )
    assert s.recall == 1.0
    assert not s.missing
    assert s.deferred == frozenset({("R101", "C12418")})
...
@pytest.mark.unit
def test_wrong_filler_still_penalized_with_expected_review_set() -> None:
    # The SME-deferred exclusion is surgical: only the expected pair is spared.
    golden = {("R101", "C12400")}
