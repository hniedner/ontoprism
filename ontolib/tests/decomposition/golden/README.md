...
The command above completed against the persisted Postgres run on 2026-09-07 after
...
The strict review-state loader also accepts a future human-selected variant bound to the same
target and candidate identities, so resolving this question does not discard or regenerate the
machine evidence. Only choice 3 may carry a candidate code, and that code must occur in the bound
16-row candidate artifact. No selected artifact is generated by software in the current state.
