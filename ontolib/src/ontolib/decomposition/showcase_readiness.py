...
class ShowcaseReadinessReport(BaseModel):
    ...
    local_graph_activated: bool
    api_ready: bool
    production_ready: bool
    scientific_publication_ready: bool
    equivalence_established: bool
    nci_adoption_asserted: bool
    decision_set_identity: str = Field(pattern=_SHA256)
    representation: str
    overlay_algorithm: str
    graph_iri: str
    source_release: str
    concept_count: int
    candidate_count: int
    ...
    collapse_policy_overlaps: dict[str, int]
...
def _report_payload(
    ...
    orthogonality = qualify_showcase_orthogonality(policy)
    ...
    return {
        ...
        "overlay_algorithm": str(policy.overlay_algorithm),
        "graph_iri": SHOWCASE_GRAPH_IRI,
        "source_release": str(policy.source_release),
        ...
        "collapse_policy_overlaps": {
            "concept_roots": len(orthogonality.concept_roots),
            "runtime_keys": len(orthogonality.runtime_keys),
            "source_occurrences": len(orthogonality.source_occurrences),
        },
