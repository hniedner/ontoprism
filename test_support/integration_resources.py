...
def remove_owned_container_by_name(
    ...
    inspected = run_docker("inspect", container_name, check=False)
...
class IntegrationConnectionPolicy:
    ...
    _allowed: set[tuple[str, int]] = field(default_factory=set)
...
class IntegrationResourceOwner:
    ...
    def postgres_run_command(self) -> list[str]:
        ...
        return [
            ...
            "--env",
            f"POSTGRES_PASSWORD={self.nonce}",
            "--env",
...
def validate_mutator_manifest_entries(
    *,
    manifest: tuple[MutatorManifestEntry, ...],
    declarations: tuple[IntegrationTestDeclaration, ...],
) -> tuple[str, ...]:
...
def validate_mutator_manifest_files(
    ...
    *,
    ...
    declarations: list[IntegrationTestDeclaration] = []
