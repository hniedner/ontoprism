...
3. Address every verified finding and every reasonable suggestion with TDD, commit, and re-run only the dimensions that have not converged, on the fix range. The only exception is a major out-of-scope finding: follow "What a finding becomes" in `AGENTS.md` and dispatch `issue-steward` for it.
