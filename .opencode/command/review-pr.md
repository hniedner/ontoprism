...
1. Dispatch all five review dimensions, never a subset: `pr-code-reviewer`, `pr-silent-failure-hunter`, `pr-comment-analyzer` and `pr-type-design-analyzer` in parallel on the milestone diff: check out the milestone branch and run `git diff --no-ext-diff main...HEAD` (the only diff form the reviewer maps allow); for a change that belongs to no milestone, the same command on its own branch. Issues inside a milestone are not reviewed individually (owner decision, 2026-09-20).
