...
1. Dispatch all five review dimensions, never a subset: `pr-code-reviewer`, `pr-silent-failure-hunter`, `pr-comment-analyzer` and `pr-type-design-analyzer` in parallel on the milestone diff (`git diff --no-ext-diff main...<milestone branch>`), or for a change that belongs to no milestone, on its diff against `main`. Issues inside a milestone are not reviewed individually (owner decision, 2026-09-20).
